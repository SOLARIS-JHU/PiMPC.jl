...
def _solve_cpu(model, x0, u0, yref, uref, w, *, warm_vars=None, verbose=False):
    ...
    w_bar = np.concatenate([np.asarray(w, dtype=dtype).reshape(-1), np.zeros(nu, dtype=dtype)])
    ...
    H_A = np.linalg.solve(Q_bar + rho * np.eye(nx_bar, dtype=dtype) + rho * (A_bar.T @ A_bar), np.eye(nx_bar, dtype=dtype))
            ...
            if Np > 1:
                X[:, 1:Np] = H_A @ (
                    ...
                    * (
                        ...
                        - Theta_hat[:, 0 : Np - 1]
                        + A_bar.T
                        @ (Z_hat[:, 1:Np] - V_hat[:, 1:Np] + Lambda_hat[:, 1:Np] - e_bar[:, None] - w_bar[:, None])
                    )
                )
            X[:, Np] = H_AN @ (q_bar_N + rho * (Z_hat[:, Np - 1] - Theta_hat[:, Np - 1]))
            BU = B_bar @ DU
            AX = A_bar @ X[:, 0:Np]
            Z = (2.0 * (X[:, 1 : Np + 1] + Theta_hat) + BU + Beta_hat + AX + e_bar[:, None] + w_bar[:, None] - Lambda_hat) / 3.0
            Z = np.minimum(np.maximum(Z, xmin_bar[:, None]), xmax_bar[:, None])
            V = 0.5 * (Z + BU + Beta_hat - AX - e_bar[:, None] - w_bar[:, None] + Lambda_hat)
            Theta = Theta_hat + X[:, 1 : Np + 1] - Z
            Beta = Beta_hat + BU - V
            Lambda = Lambda_hat + Z - AX - V - e_bar[:, None] - w_bar[:, None]
            ...
            if Np > 1:
                X[:, 1:Np] = H_A @ (
                    ...
                    * (
                        Z[:, 0 : Np - 1]
                        - Theta[:, 0 : Np - 1]
                        + A_bar.T @ (Z[:, 1:Np] - V[:, 1:Np] + Lambda[:, 1:Np] - e_bar[:, None] - w_bar[:, None])
                    )
                )
            X[:, Np] = H_AN @ (q_bar_N + rho * (Z[:, Np - 1] - Theta[:, Np - 1]))
            BU = B_bar @ DU
            AX = A_bar @ X[:, 0:Np]
            Z = (2.0 * (X[:, 1 : Np + 1] + Theta) + BU + Beta + AX + e_bar[:, None] + w_bar[:, None] - Lambda) / 3.0
            Z = np.minimum(np.maximum(Z, xmin_bar[:, None]), xmax_bar[:, None])
            V = 0.5 * (Z + BU + Beta - AX - e_bar[:, None] - w_bar[:, None] + Lambda)
            Theta = Theta + X[:, 1 : Np + 1] - Z
            Beta = Beta + BU - V
            Lambda = Lambda + Z - AX - V - e_bar[:, None] - w_bar[:, None]
...
def _solve_gpu(model, x0, u0, yref, uref, w, *, warm_vars=None, verbose=False):
    ...
    w_bar = cp.asarray(np.concatenate([np.asarray(w, dtype=model.dtype).reshape(-1), np.zeros(nu, dtype=model.dtype)]), dtype=dtype)
    ...
    for iter_idx in range(1, max_iter + 1):
        ...
        if model.accel:
            ...
            BU = B_bar @ DU
            AX = A_bar @ X[:, 0:Np]
            Z = (dtype(2.0) * (X[:, 1 : Np + 1] + Theta_hat) + BU + Beta_hat + AX + e_bar[:, None] + w_bar[:, None] - Lambda_hat) / dtype(3.0)
            Z = cp.minimum(cp.maximum(Z, xmin_bar[:, None]), xmax_bar[:, None])
            V = dtype(0.5) * (Z + BU + Beta_hat - AX - e_bar[:, None] - w_bar[:, None] + Lambda_hat)
            Theta = Theta_hat + X[:, 1 : Np + 1] - Z
            Beta = Beta_hat + BU - V
            Lambda = Lambda_hat + Z - AX - V - e_bar[:, None] - w_bar[:, None]
            ...
            BU = B_bar @ DU
            AX = A_bar @ X[:, 0:Np]
            Z = (dtype(2.0) * (X[:, 1 : Np + 1] + Theta) + BU + Beta + AX + e_bar[:, None] + w_bar[:, None] - Lambda) / dtype(3.0)
            Z = cp.minimum(cp.maximum(Z, xmin_bar[:, None]), xmax_bar[:, None])
            V = dtype(0.5) * (Z + BU + Beta - AX - e_bar[:, None] - w_bar[:, None] + Lambda)
            Theta = Theta + X[:, 1 : Np + 1] - Z
            Beta = Beta + BU - V
            Lambda = Lambda + Z - AX - V - e_bar[:, None] - w_bar[:, None]
